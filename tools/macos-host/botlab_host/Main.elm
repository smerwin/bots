...
compiling, so any bot written against `BotLab.BotInterface_To_Host_2024_10_19`
...
import Bot
import BotLab.BotInterface_To_Host_2024_10_19 as IH
...
encodeTask task =
    case task of
        ...
        IH.WindowsInputRequest items ->
            E.object [ ( "WindowsInputRequest", E.list encodeWindowsInputSequenceItem items ) ]
...
encodeMethodOnWindow m =
    case m of
        ...
        IH.SetZoomFactorOnWebViewMethod p ->
            E.object [ ( "SetZoomFactorOnWebViewMethod", E.object [ ( "zoomFactorMikro", E.int p.zoomFactorMikro ) ] ) ]

        IH.ReadFromWindowMethod ->
            E.object [ ( "ReadFromWindowMethod", E.null ) ]


encodeWindowsInputSequenceItem : IH.WindowsInputSequenceItem -> E.Value
encodeWindowsInputSequenceItem item =
    case item of
        IH.WaitMilliseconds ms ->
            E.object [ ( "WaitMilliseconds", E.int ms ) ]

        IH.KeyDown code extended ->
            E.object [ ( "KeyDown", E.list identity [ E.int code, E.bool extended ] ) ]

        IH.KeyUp code extended ->
            E.object [ ( "KeyUp", E.list identity [ E.int code, E.bool extended ] ) ]

        IH.MouseMoveAbsolute x y ->
            E.object [ ( "MouseMoveAbsolute", E.list E.int [ x, y ] ) ]

        IH.MouseMoveRelative x y ->
            E.object [ ( "MouseMoveRelative", E.list E.int [ x, y ] ) ]

        IH.ButtonDown b ->
            E.object [ ( "ButtonDown", E.int b ) ]

        IH.ButtonUp b ->
            E.object [ ( "ButtonUp", E.int b ) ]

        IH.ButtonScroll b dir off ->
            E.object [ ( "ButtonScroll", E.list E.int [ b, dir, off ] ) ]

        IH.CharacterDown c ->
            E.object [ ( "CharacterDown", E.int c ) ]

        IH.CharacterUp c ->
            E.object [ ( "CharacterUp", E.int c ) ]

        IH.BringWindowToForeground windowId ->
            E.object [ ( "BringWindowToForeground", E.string windowId ) ]

        IH.AbortIfWindowNotInForeground windowId ->
            E.object [ ( "AbortIfWindowNotInForeground", E.string windowId ) ]
...
decodeTaskResult =
    D.oneOf
        ...
        , D.field "WindowsInputResponse" decodeWindowsInputResponse |> D.map IH.WindowsInputResponse
...
decodeReadFromWindowComplete =
    D.map7
        (\readingId windowText windowRect clientRect clientRectLeftUpperToScreen windowDpi imageData ->
            { readingId = readingId
...
decodeWindowsInputResponse : D.Decoder IH.WindowsInputResponseStruct
decodeWindowsInputResponse =
    D.map4 IH.WindowsInputResponseStruct
        (D.field "completedStepsCount" D.int)
        (D.field "abortedStepsCount" D.int)
        (D.field "totalTimeMilliseconds" D.int)
        (D.field "errorMessages" (D.list D.string))
