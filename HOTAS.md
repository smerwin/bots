...
Pinned. Nothing implemented. The client-side facts above were measured live on
2026-08-02 during run 5 and are the parts most likely to be wrong later:
widget positions move with layout, and `SpeedGauge`/`MaxSpeedButton` type names
are the stable handles rather than the coordinates.
